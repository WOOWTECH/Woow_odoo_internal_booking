...
### 提案解決方案
開發一個獨立的資源預約模組，擴展 Odoo 18 日曆功能，支援 Portal 用戶透過 Portal 介面預約資源。管理員可在資源設定中指定哪些聯絡人（Portal 用戶）有權預約該資源，且不依賴網站 (website) 模組。
...
#### US-004: Portal 用戶查看可預約資源
```
作為 被授權的 Portal 用戶
我想要 在 Portal 頁面看到我被授權預約的資源
以便 選擇合適的資源進行預約

驗收條件:
- [ ] 只能看到我被授權預約的資源
- [ ] 可查看資源的詳細資訊（位置、容量、說明等）
- [ ] 可查看資源的可用時段
- [ ] 可直接從列表進入預約流程
```
...
| FR-004 | 聯絡人權限分享 | P0 | 在資源表單設定可預約的聯絡人 |
| FR-005 | Portal 資源列表頁 | P0 | 僅顯示被授權的資源 |
| FR-006 | Portal 預約表單 | P0 | 日期時間選擇器 |
| FR-007 | Portal 我的預約頁 | P0 | 列表與日曆視圖 |
| FR-008 | 預約衝突檢測 | P0 | 即時檢測 |
| FR-009 | 預約狀態管理 | P1 | 待確認/已確認/已取消 |
| FR-010 | 郵件通知 | P1 | 確認/提醒/取消 |
| FR-011 | 預約審核 | P1 | 可選功能 |
| FR-012 | 日曆事件同步 | P1 | calendar.event |
| FR-013 | 報表統計 | P2 | 使用率統計 |
...
class BookingResource(models.Model):
    ...
    share_type = fields.Selection([
        ('all', 'All Portal Users'),      # 所有 Portal 用戶皆可預約
        ('specific', 'Specific Contacts'), # 僅指定聯絡人可預約
    ], default='specific', required=True, string='Access Type')
    allowed_partner_ids = fields.Many2many(
        'res.partner',
        'booking_resource_partner_rel',
        'resource_id',
        'partner_id',
...
class BookingReservation(models.Model):
    ...
    @api.constrains('resource_id', 'partner_id')
    def _check_partner_access(self):
        """確保預約人有權預約該資源"""
        for record in self:
            if not record.resource_id._check_partner_access(record.partner_id):
...
# /my/bookings/new/<resource_id> - 新增指定資源的預約
...
def _get_accessible_resources(self, partner):
    """取得該聯絡人可預約的資源"""
    return self.env['booking.resource'].sudo().search([
        '|',
        ('share_type', '=', 'all'),
...
| 里程碑 | 預估週數 | 交付項目 |
|--------|----------|----------|
| M1: 資料模型 | Week 1-2 | 模型定義、後台視圖、聯絡人權限欄位 |
| M2: 後台功能 | Week 3-4 | 資源管理、時段設定、權限設定 UI |
